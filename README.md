# BrainWave: Retrieval augmented generation for middle-school math question answering and hint generation


How can we incorporate trusted, external math knowledge in generated answers to student questions?

`BrainWave` is a Python package that implements basic retrieval augmented generation (RAG) and contains prompts for two primary use cases: general math question-answering (QA) and hint generation. It is currently designed to work only with the OpenAI generative chat API.


## Demo
```bash
You can explore the effects of the retrieval-augmented generation approach by using our [Streamlit app](https://BrainWave.streamlit.app/About_this_app). You'll need to provide your own OpenAI API key.
Example
...
The `BrainWave` package is [available on PyPI](https://pypi.org/project/BrainWave/).
...
We'll use `BrainWave` to answer a student question.
```python
...
Note: the `brain_wave.openstax` module relies on `requests` and `beautifulsoup4`, which are not listed as dependencies. Install them yourself with `pip` if you want to download and parse OpenStax textbooks.

```python
from brain_wave import openstax
...
```python
from brain_wave import retrieval_strategies
...
```python
from brain_wave import prompt_utils
...
```python
from brain_wave.prompts import mathqa as mathqa_prompts
